...
class Simulator:
    ...
    def __init__(self, case_name="test_traj_3D",data_file="train_data", reversed=False):
        ...
        self.data_root = os.path.join(Script_Root, "DATA", self.case_name)
    ...
    def _load_data(self,data_file):
        ...
        self.data_x = torch.tensor(np.hstack((kappa, lf, n, alpha))).to(self.data_type)
    ...
    def update(self, u:torch.tensor):
        kappa, _, n, alpha, v = self.x.numpy()[0]
    ...
    def start_sim(self):
        ...
        for i in range(iter):
            x_value = self.x[0, 0:4].unsqueeze(0)
    ...
    def save_data(self):
        heads = ['kappa', 's', 'look_fw_curvature', 'n', 'alpha', 'v', "v_comm", "delta"]
