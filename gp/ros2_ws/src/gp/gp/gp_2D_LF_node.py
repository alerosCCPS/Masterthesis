...
class Controller_GP2D_LF(Node):
    ...
    def __init__(self, name):
        ...
        # self.data_root = os.path.join(Script_Root, "DATA", "test_traj_2D_LF")
        self.data_root = os.path.join(Script_Root, "DATA", f"val_traj_2D_LF")
        self.mpc = GP2D_LF(self.data_root)
        ...
        self.x = torch.tensor([[0, 0,0]]).to(self.data_type)  # n, alpha,
    ...
    def track_callback(self, msg):
        ...
        self.x[0, 0] = torch.tensor(msg.curvature_lf)
...
def start_gp_2D_LF(args=None):

    rclpy.init(args=args)

    gp_2D_LF = Controller_GP2D_LF(name='gp_2D_LF')
    gp_2D_LF.get_logger().info("create handle of comm_pub_handle")
    try:
        rclpy.spin(node=gp_2D_LF)
    except KeyboardInterrupt:
        pass
    finally:
        gp_2D_LF.save_data()
        gp_2D_LF.destroy_node()
        rclpy.shutdown()