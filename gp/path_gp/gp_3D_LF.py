...
class GP(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        ...
        self.covar_module = ScaleKernel(RBFKernel(ard_num_dims=4))
        # self.covar_module = ScaleKernel(MaternKernel(nu=2.5, ard_num_dims=4))
...
class Trainer:
    ...
    def plot_feature_space(self, e):
        ...
        sc = ax.scatter(data[:,0], data[:,2], data[:,3], c=np.abs(e), cmap=cm, norm=norm, marker='o', s=5)
    ...
    def save_model(self):
        ...
        }, os.path.join(self.data_root,self.case_name, 'gp_3D_LF.pth'))
    ...
    def load_data(self, path):
        ...
        x = torch.tensor(np.hstack((kappa, lf, n, alpha))).to(self.data_type)
...
if __name__ == '__main__':
    ...
    # ax = fig.add_subplot(111, projection='3d')
    # data =[1,1,1]
    # sc = ax.scatter(data[0], data[1], data[2], marker='o', s=5)
    # plt.show()
