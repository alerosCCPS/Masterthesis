...
def get_hamster_model(path_file_name='circle'):
    ...
    model.L = model.length_front + model.length_rear 
    model.T = 0.00984

    def dynamic_f(x, u, kappa):
        s, n, alpha, v = x[0], x[1], x[2], x[3]
        # kappa = interpolator(s)
        delta, v_command = u[0], u[1]
        
        
        #beta = 0.5*delta
        s_dot = v*ca.cos(alpha)/(1-n*kappa)
        n_dot = v*ca.sin(alpha)
        alpha_dot = v*(ca.tan(delta)/model.L)-kappa*s_dot#v*ca.sin(beta)/model.length_rear - kappa*s_dot
    ...
    u = ca.vertcat(delta, v_comm) #ca.vertcat(v_comm, delta)
    ...
    constrains.s_limit = path_length #ca.inf#path_length  # total curve length
    constrains.n_limit = ca.inf#0.1  # distance bias
    constrains.alpha_limit = ca.inf#ca.pi*0.5
