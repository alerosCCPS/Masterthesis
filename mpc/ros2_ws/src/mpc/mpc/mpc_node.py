...
def get_hamster_model(path_file_name='test_traj_normal'):
    _, _, path_length = load_path(path_file_name)
    ...
    def dynamic_f(x, u, kappa):
        ...
        delta = model.steer_k * delta + (model.length_rear + model.length_front) * model.steer_c
        beta = model.length_rear *delta/(model.length_rear + model.length_front)
...
class MPC_Adapted:
    ...
    def __init__(self, path_name='test_traj'):
        ...
        self.R = np.diag([1e-3, 1e-3, 1e-1, 1e-1])
        self.J = 0
    ...
    def setup_nlp(self):
        ...
        for i in range(self.N):
            ...
            # alpha_ref = -ca.asin((kappa-self.hamster.steer_c) * l_r/self.hamster.steer_k)
            # delta_ref = L/l_r * ca.asin((kappa-self.hamster.steer_c)*l_r/self.hamster.steer_k)
            alpha_ref = -l_r*(kappa-self.hamster.steer_c)/self.hamster.steer_k
            delta_ref = L*(kappa-self.hamster.steer_c)/self.hamster.steer_k
