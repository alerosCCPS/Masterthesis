...
class MPC:
    ...
    def __init__(self, path_name='test_traj'):
        ...
        self.No_track = False
        self.kappa_ref = 1
        self.x_init = [0, 0, 0, 0]  # s, n, alpha, v
    ...
    def predict(self, x0=ca.repmat(0, 4 * (20 + 1) + 2 * 20, 1), x=[0, 0, 0, 0]):
        ...
        x0_res = res["x"]
        con = ca.reshape(x0_res[self.nx * (self.N + 1):], self.nu,
                         self.N).full()  # planned control input (due to nlp formulation they are stored at the end of the solution vector)
    ...
    def sim(self):
        ...
        x0u = ca.repmat([self.constraints.v_limit,ca.atan( l_r * kappa)], self.N)
